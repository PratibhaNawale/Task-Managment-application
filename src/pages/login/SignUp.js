...
import { Button, TextField, Grid, Typography, Box, Paper, InputLabel, IconButton, InputAdornment } from '@mui/material';
...
const SignUp = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.error('Error occurred during sign up. Please check your network.', {
        style: { backgroundColor: 'red', color: 'white' },
      });
    ...
    <Grid container sx={{ height: '100vh' }}>
      <Grid item xs={12} md={6} className="login-bg">
        <Typography className='text-light text-center fontw700' variant='h4'>
          Create A Account In Task Management Application
        </Typography>
      </Grid>
      <Grid item xs={12} md={6} component={Paper} elevation={3} className="form-container">
        <Box className="form-box">
          <Typography className="fontw700" variant="h4" gutterBottom>
            Sign Up
          </Typography>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Typography className='mt-2'>Already registered?<Link to='/signin' className='mt-2'> SignIn</Link></Typography>
